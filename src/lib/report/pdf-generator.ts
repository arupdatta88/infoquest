...
export async function generatePDF(result: ResearchResult): Promise<Uint8Array> {
  ...
  // Embed SolaimanLipi for Bengali text - try src/fonts/ first (bundled, Vercel-compatible),
  // then fall back to public/fonts/ (local dev)
  let bengaliFont: any = font;
  try {
    const fontBytes = readFileSync(join(process.cwd(), 'src', 'fonts', 'SolaimanLipi.ttf'));
